...
class MembershipList:
    def __init__(self, introducor, port_num):
        ...
        self.introducor = 'fa23-cs425-8002.cs.illinois.edu'
        self.MembershipList = {
            ...
            for ip, port in [(IP, self.port) for IP in [self.ip, self.introducor]]
        }
        self.leader = 'fa23-cs425-8002.cs.illinois.edu'
        self.presidency = 0
        # List to track failed members.
        self.failMemberList = {}
        self.suspect_members_detected = []
    ...
    def returnDetectedKey(self):
        with self.rlock:
            return [key.split(':')[0] for key in self.MembershipList.keys()]
    def printID(self):
        # Method to print the unique ID of the server.
        with self.rlock:
            print(self.id)
    def printSuspisionNode(self):
        with self.rlock:
            if self.gossipS == True:
                print("Suspision Node: ")
                for i in self.suspect_members_detected:
                    print(i)
            else:
                print("Not in a suspision mode.")
        ...
        with self.rlock:
            ...
            for member_id, member_info in membershipList.items():
                ...
                if member_id in self.failMemberList:
                    ...
                    continue
                if member_info['presidency'] > self.presidency:
                    self.presidency = member_info["presidency"]
                    self.leader = member_info["leader"]
                if member_info['presidencyS'] > self.presidencyS:
                    self.presidencyS = member_info["presidencyS"]
                    self.gossipS = member_info["gossip_S"]
                ...
                if member_id in self.MembershipList:
                    ...
                    # Update only if the received heartbeat is greater and both at the same incarnation
                        ...
                        self.MembershipList[member_id]["id"] = member_info["id"]
                        self.MembershipList[member_id]["addr"] = member_info["addr"]
                        self.MembershipList[member_id]["heartbeat"] = member_info["heartbeat"]
                        self.MembershipList[member_id]["time"] = time.time()
    ...
    def detectSuspectAndFailMember(self):
        ...
        with self.rlock:
            ...
            self.suspect_members_detected = [member_id for member_id, member_info in self.MembershipList.items() if member_info['time'] < failure_threshold_time and member_info["status"] != "Suspect"]
            for member_id in self.suspect_members_detected:
            ...
            # If using GossipS protocol, include additional information like status and incarnation.
                return {
                    m['id']:{
                        ...
                        "presidencyS": self.presidencyS,
                        "gossip_S": self.gossipS
                    }
